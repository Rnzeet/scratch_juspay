  ...
  return (
    <div className="w-60 flex-none h-full overflow-y-auto flex flex-col items-start p-2 border-r border-gray-200">
      ...
      <button draggable={true}
        onDragStart={(event) => handleDragStart(event, "Move 10 steps")} onClick={() =>{ handleClick(`ml-${a}`)}} className="flex flex-row flex-wrap bg-blue-500 text-white px-2 py-1 my-2 text-sm cursor-pointer after:content-['*'] after:bg-red-500 after:px-2 after:py-2 after:my-2 before:content-['*'] before:bg-red-500 before:px-1 before:py-1 before:my-1" >
        {"Move "}
        <div className="mx-2 py-1 px-2 bg-white rounded-md text-black " >{"10"}</div>{"steps"}
      </button>
      <button draggable={true}
        onDragStart={(event) => handleDragStart(event, "Turn 15 degrees")} onClick={() =>{ handleClick(` transform rotate-${b}`)}} className="flex flex-row flex-wrap bg-blue-500 text-white px-2 py-1 my-2 text-sm cursor-pointer">
        {"Turn "}
        <Icon name="undo" size={15} className="text-white mx-2" />
        <div className="mx-2 py-1 px-2 bg-white rounded-md text-black" >{"15"}</div> {"degrees"}
      </button>
      <button draggable={true}
        onDragStart={(event) => handleDragStart(event, "Turn 15 degrees")}  onClick={() =>{ handleClick(`transform rotate-${c} `)}} className="flex flex-row flex-wrap bg-blue-500 text-white px-2 py-1 my-2 text-sm cursor-pointer">
        {"Turn "}
        <Icon name="redo" size={15} className="text-white mx-2" />
        <div className="mx-2 py-1 px-2 bg-white rounded-md text-black" >{"15"}</div> {"degrees"}
      </button>
      <button draggable={true}
        onDragStart={(event) => handleDragStart(event, "go to Select option")} onClick={() =>{ handleClick(`mx-${Math.floor(Math.random())} my-${Math.floor(Math.random())}`)}} className="flex flex-row flex-wrap bg-blue-500 text-white px-2 py-1 my-2 text-sm cursor-pointer">
        {"go to"}
